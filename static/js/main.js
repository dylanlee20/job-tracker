...
function triggerScrape() {
  ...
  $.ajax({
    ...
    success: function (tokenResponse) {
      ...
      // Now make the scrape request with CSRF token
      $.ajax({
        url: "/api/scrape",
        method: "POST",
...
function showScrapeProgressModal() {
  ...
  const modal = new bootstrap.Modal(
    document.getElementById("scrapeProgressModal"),
  );
  modal.show();
}
...
function updateProgressUI(progress) {
  ...
  $("#scrapeCurrentCompany").text(
    progress.current_company
      ? `Scraping: ${progress.current_company}`
      : "Starting...",
  );
  ...
  if (
    progress.current_company &&
    !completed.includes(progress.current_company) &&
    !failed.includes(progress.current_company)
  ) {
    listHtml += `<div class="text-primary"><i class="bi bi-arrow-repeat me-1 spin"></i>${progress.current_company}</div>`;
  }
  $("#scrapeCompanyList").html(
    listHtml || '<div class="text-muted">Waiting...</div>',
  );
